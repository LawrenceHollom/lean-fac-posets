...
6. **Construct stable data before maximal extensions.** A sequential upper-bound proof must first
   ...
   Only then may it extend that union to a maximal chain. This avoids the circular limit argument
   found and repaired in Stage 2.
...
- [x] **S3.C.5 — tail and head equivalence.** Define increasing equivalence through mutually
  ...
  these setoids.
- [ ] **S3.C.6 — setoid laws (high).** Prove transitivity by aligning the two middle segments,
  restricting away finite boundary classes, and composing the class isomorphisms. Split this into
  lemmas for segment alignment, boundary removal, finite incomparability, and convex-hull
  maximality before assembling the theorem.
- [ ] **S3.C.7 — germ types.** Define increasing and decreasing quotient germs and expose equality,
  representative, direction, and duality APIs.
- [ ] **S3.C.8 — API gate.** In a small test module, prove representative-independent statements
  using only public declarations; then mark the setoids and quotient constructors private.

### Phase D — construction and order of `H(P)`

- [ ] **S3.D.1 — normalized point type.** In `Completion/Construction.lean`, define `H α` with
  constructors for `principal x`, a nonprincipal increasing germ, a nonprincipal decreasing germ,
  `⊥`, and `⊤`.
- [ ] **S3.D.2 — nonprincipal views.** Define `N`, `N+`, and `N-` as direction-tagged subtypes,
  placing `⊤` in `N+` and `⊥` in `N-`; provide exhaustive case and disjointness lemmas.
- [ ] **S3.D.3 — representative relation.** In `Completion/Representatives.lean`, define when a
...
- [ ] **S3.D.4 — principal embedding.** Define `principal : α → H α`, prove injectivity, and add simp
  lemmas for all constructors. Do not yet declare it an order embedding.
- [ ] **S3.D.5 — representative-level domination (high).** Define strict domination for every
  direction pair using nonempty tails/heads. Prove invariance under chain equivalence before
  lifting it to germs.
- [ ] **S3.D.6 — mixed domination cases.** Prove principal/germ, germ/principal,
  increasing/decreasing, and endpoint formulas. Use order duality to halve the cases.
- [ ] **S3.D.7 — strict-order laws (high).** Prove irreflexivity and transitivity of domination,
  including all mixed constructors.
- [ ] **S3.D.8 — partial order.** Define `X ≤ Y` by equality or domination, build the `PartialOrder`
  instance, and prove useful `lt_iff` and constructor comparison simp lemmas.
- [ ] **S3.D.9 — principal order embedding.** Upgrade `principal` to `α ↪o H α` and prove it
  preserves/reflection comparability and incomparability.
- [ ] **S3.D.10 — cuts induced by germs.** Characterize the principal points below/above a germ;
  prove separation of two distinct comparable germs by a principal point when required.
- [ ] **S3.D.11 — scattering of `H(P)`.** Use S3.A.5 and principal separators to prove
  `IsScattered α → IsScattered (H α)`.
- [ ] **S3.D.12 — order gate.** Check `#print axioms` for the order and scattering theorems and add
  examples for finite chains, `ℕ`, `ℤ`, and their duals.

### Phase E — sub-completions and completeness

- [ ] **S3.E.1 — admissible subposets.** In `Completion/Embedding.lean`, define the condition that
  saturated chains of `Q` remain saturated in `P`, and prove it for convex subposets and saturated
  chains.
- [ ] **S3.E.2 — map germs along an inclusion.** Lift representatives, prove preservation and
  reflection of chain equivalence, and define the induced map `H(Q) → H(P)`.
- [ ] **S3.E.3 — completion embedding.** Prove the induced map is an order embedding, commutes with
  principal points and endpoints, and preserves direction and representatives.
- [ ] **S3.E.4 — completion of a chain is linear.** In `Completion/Completeness.lean`, prove
  `H(C)` is linearly ordered and prove the principal-separator lemma between increasing germs.
- [ ] **S3.E.5 — explicit sup candidate (high).** For `D : Set (H C)` without a maximum, define the
  initial segment of principal points lying below some member of `D`; prove it is saturated and has
  no maximum.
- [ ] **S3.E.6 — least-upper-bound theorem.** Show the increasing germ of that segment is `sSup D`,
  with separate proofs for empty sets, sets with maxima, and the endpoint cases.
- [ ] **S3.E.7 — infimum by duality.** Derive the dual construction and install the
  `CompleteLinearOrder (H C)` instance.
- [ ] **S3.E.8 — shape of non-attained bounds.** Prove that a supremum not attained by its set lies
  in `N+`, and dually for infima. Expose sequence-specialized versions for later files.
- [ ] **S3.E.9 — continuity toolkit.** Define the sup/inf-preserving order embeddings prototyped in
  S3.0.6, with identity, composition, restriction, cofinal-image, and coinitial-image lemmas.
- [ ] **S3.E.10 — accumulation and bicomparability.** Define accumulation/isolated nonprincipal
...
- [ ] **S3.E.11 — completion gate.** Freeze the `H(P)` API. No later module may unfold germ
...
- [ ] **S3.I.5 — completion embedding witness.** In `Consolidation/Relation.lean`, package a strict
  order embedding `N(C) ↪o N(D)` with direction preservation, the needed sup/inf continuity, and
  cofinal/coinitial image.
...
- [ ] **S3.I.7 — well-definedness on germs (high).** Prove consolidation is invariant under change
  of representatives in both arguments before introducing notation on nonprincipal points.
- [ ] **S3.I.8 — cofinality lemma.** Prove every consolidation has the underlying cofinal/coinitial
  behavior claimed in `lem:consolidation-is-cofinal`.
...
- [ ] **S3.I.11 — vacillating antisymmetry (critical).** Iterate the composite self-embedding;
  show a moved `N+` point or `N-` point creates a forbidden vacillating chain, then handle the atomic
  two-sided case separately.
- [ ] **S3.I.12 — partial order.** Install the consolidation partial order on the normalized germ
  or candidate object actually used later; avoid overloaded notation on raw chains.
...
- [ ] **S3.J.4 — wellfounded `N+`.** Prove `N+` of a countable scattered vacillating
  out-wellfounded poset is well founded; derive the dual statement for `N-`.
- [ ] **S3.J.5 — tagged atomic transition graph (critical).** For a sequence of candidate
  consolidations, define stage-tagged atomic states and transition witnesses. Prove every infinite
  path eventually stabilizes using the domination antichain lemma and S3.J.4.
- [ ] **S3.J.6 — stable atomic points.** Construct stabilization points and their suprema before
  choosing any limit maximal chain. Prove orderedness, direction, atomicity in the eventual chain,
  and independence from representative choices.
- [ ] **S3.J.7 — stable-limit chain (critical).** Prove the liminf of the candidate carriers is
  nonempty and contains all required stable intervals. Only after these results extend it to a
  maximal chain. This deliberately corrects the order of the manuscript's construction.
- [ ] **S3.J.8 — limit embedding on atomic points.** Define the image of an atomic point as the
  appropriate supremum/infimum of its stabilization set and prove strict order preservation.
...
- [ ] **S3.J.10 — continuity and cofinality.** Prove the limit map preserves the required
  suprema/infima and has cofinal/coinitial image.
...
- [ ] **S3.J.14 — illfounded endpoint preservation.** Prove consolidation carries cofinal
  alternating `N+`/`N-` sequences to an illfounded bicomparable endpoint.
...
7. **After S3.J.13:** sequential consolidation limits and the passage to arbitrary chains must be
   separate checked theorems. No maximal extension may be selected before the stable-limit chain
   is proved nonempty.
